...
public class LastServlet extends HttpServlet
{
	public void service(HttpServletRequest req,HttpServletResponse res) throws ServletException,IOException
	{
		...
		String trans=btablename+"transaction";
		while(en.hasMoreElements())
			...
			st1.executeUpdate("insert into transactions values("+"'"+arr[0]+"'"+","+Integer.parseInt(arr[1])+","+"'"+strDate+"'"+")");
		 }


		  ResultSet rs=st.executeQuery("select sum(value),reg_date from transactions group by reg_date order by reg_date");
		 int i=0;
		  while(rs.next())
		 {
